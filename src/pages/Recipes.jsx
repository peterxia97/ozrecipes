      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
          {filtered.map(recipe => {
            ...
            const diffColor = difficultyColor[recipe.difficulty] || 'bg-gray-400';
            return (
              <Link
                key={recipe.id}
              ...
              >
                ...
                <div className="h-28 sm:h-44 bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center text-5xl sm:text-7xl relative overflow-hidden">
                  ...
                  {/* Cook time badge */}
                  <span className="absolute bottom-2 left-2 sm:bottom-3 sm:left-3 bg-black/50 text-white text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-full backdrop-blur-sm">
                    ⏱️ {recipe.cookTime}分钟
                  </span>
                </div>
                <div className="p-2.5 sm:p-4">
                  <h3 className="font-bold text-xs sm:text-sm text-gray-900 mb-1 line-clamp-1 sm:line-clamp-2">{recipe.name}</h3>
